...
describe('MIS Vesting contract unit testing', function() {
	describe('Team Vesting', function () {
		...
		beforeEach(async function() {
			...
			await expect(erc20Contract.transfer(teamVestingContract.address, ethers.utils.parseEther('100000000000')))
				.to.emit(teamVestingContract, 'TokenReceive')
		...
		it ('Reverts execution of vesting function when paused', async function() {
			await expect(teamVestingContract.pause())
				.to.emit(teamVestingContract, 'Paused')
				.withArgs(owner.address);
			await expect(teamVestingContract.vesting(new Date(2022, 1, 18).getTime() / 1000, addr1.address, ethers.utils.parseEther('10000000000')))
		...
		it ('Reverts execution of withdraw function when paused', async function() {
			await expect(teamVestingContract.pause())
				.to.emit(teamVestingContract, 'Paused')
				.withArgs(owner.address);
			await expect(teamVestingContract.withdraw(addr1.address))
				.to.be.revertedWith('Pausable: paused');
		})
		it ('Reverts execution of claimableAmount function when paused', async function() {
			await expect(teamVestingContract.pause())
				.to.emit(teamVestingContract, 'Paused')
				.withArgs(owner.address);
			await expect(teamVestingContract.claimableAmount(addr1.address))
				.to.be.revertedWith('Pausable: paused');
		})
		it('Reverts during vesting by owner, when the TOTAL_AMOUNT is already vested', async function() {
			...
			await expect(teamVestingContract.vesting(new Date(2022, 2, 18).getTime() / 1000, addr2.address, ethers.utils.parseEther('30000000000')))
				.to.emit(teamVestingContract, 'TokenVest')
				.withArgs(ethers.utils.parseEther('40000000000'));
			await expect(teamVestingContract.vesting(new Date(2022, 3, 18).getTime() / 1000, addr3.address, ethers.utils.parseEther('40000000000')))
				.to.emit(teamVestingContract, 'TokenVest')
				.withArgs(ethers.utils.parseEther('80000000000'));
			await expect(teamVestingContract.vesting(new Date(2022, 4, 18).getTime() / 1000, addr4.address, ethers.utils.parseEther('20000000000')))
				.to.emit(teamVestingContract, 'TokenVest')
				.withArgs(ethers.utils.parseEther('100000000000'));
			await expect(teamVestingContract.vesting(new Date(2022, 4, 18).getTime() / 1000, addr1.address, ethers.utils.parseEther('20000000000')))
				.to.be.revertedWith('TOTAL_AMOUNT is already vested');
		});
		it('addr1 successfully claim tokens', async function() {
			await expect(teamVestingContract.vesting(new Date(2021, 1, 18).getTime() / 1000, addr1.address, ethers.utils.parseEther('10000000000')))
				.to.emit(teamVestingContract, 'TokenVest')
				.withArgs(ethers.utils.parseEther('10000000000'));
			await expect(teamVestingContract.withdraw(addr1.address))
	...
	describe('In-App Staking', function () {
		...
		beforeEach(async function() {
			...
			await expect(erc20Contract.transfer(stakingContract.address, ethers.utils.parseEther('100000000000')))
				.to.emit(stakingContract, 'TokenReceive')
				.withArgs(ethers.utils.parseEther('100000000000'));
		});
		...
		it ('Reverts execution of withdraw function when paused', async function() {
			await expect(stakingContract.pause())
				.to.emit(stakingContract, 'Paused')
				.withArgs(owner.address);
			await expect(stakingContract.withdraw())
				.to.be.revertedWith('Pausable: paused');
		})
		it ('Reverts execution of availableAmount function when paused', async function() {
			await expect(stakingContract.pause())
				.to.emit(stakingContract, 'Paused')
				.withArgs(owner.address);
			await expect(stakingContract.availableAmount())
				.to.be.revertedWith('Pausable: paused');
		})
		it('owner is able to change a releaseTime', async function() {
			await expect(stakingContract.setReleaseTime(new Date(2021, 1, 18).getTime() / 1000))
				.to.emit(stakingContract, 'ReleaseTimeChange')
				.withArgs(new Date(2021, 1, 18).getTime() / 1000);
		});
		it('Reverts, when owner claim before release time', async function() {
			await expect(stakingContract.withdraw())
		...
		it('owner successfully claim tokens', async function() {
			await expect(stakingContract.setReleaseTime(new Date(2021, 1, 18).getTime() / 1000))
				...
				.withArgs(new Date(2021, 1, 18).getTime() / 1000);
			await expect(stakingContract.withdraw())
				.to.emit(stakingContract, 'TokenWithdraw')
				.withArgs(ethers.utils.parseEther('100000000000'));
	...
	describe('Marketing', function () {
		...
		beforeEach(async function() {
			...
			await expect(erc20Contract.transfer(marketingContract.address, ethers.utils.parseEther('50000000000')))
				.to.emit(marketingContract, 'TokenReceive')
		...
		it ('Reverts execution of availableAmount function when paused', async function() {
			await expect(marketingContract.pause())
				.to.emit(marketingContract, 'Paused')
				.withArgs(owner.address);
			await expect(marketingContract.availableAmount(addr1.address))
				.to.be.revertedWith('Pausable: paused');
		})
		it('Reverts during vesting by owner, when the TOTAL_AMOUNT is already vested', async function() {
			await expect(marketingContract.vesting(new Date(2022, 1, 18).getTime() / 1000, addr1.address, ethers.utils.parseEther('5000000000')))
				.to.emit(marketingContract, 'TokenVest')
				.withArgs(ethers.utils.parseEther('5000000000'));
			await expect(marketingContract.vesting(new Date(2022, 2, 18).getTime() / 1000, addr2.address, ethers.utils.parseEther('10000000000')))
				.to.emit(marketingContract, 'TokenVest')
				.withArgs(ethers.utils.parseEther('15000000000'));
			await expect(marketingContract.vesting(new Date(2022, 3, 18).getTime() / 1000, addr3.address, ethers.utils.parseEther('30000000000')))
				.to.emit(marketingContract, 'TokenVest')
				.withArgs(ethers.utils.parseEther('45000000000'));
			await expect(marketingContract.vesting(new Date(2022, 4, 18).getTime() / 1000, addr4.address, ethers.utils.parseEther('5000000000')))
				.to.emit(marketingContract, 'TokenVest')
				.withArgs(ethers.utils.parseEther('50000000000'));
			await expect(marketingContract.vesting(new Date(2022, 4, 18).getTime() / 1000, addr1.address, ethers.utils.parseEther('5000000000')))
				.to.be.revertedWith('TOTAL_AMOUNT is already vested');				
		});
		it('addr1 successfully claim tokens', async function() {
			await expect(marketingContract.vesting(new Date(2021, 1, 18).getTime() / 1000, addr1.address, ethers.utils.parseEther('10000000000')))
				...
				.withArgs(ethers.utils.parseEther('10000000000'));
			await expect(marketingContract.withdraw(addr1.address))
				.to.emit(marketingContract, 'TokenWithdraw')
				.withArgs(ethers.utils.parseEther('10000000000'));
			expect(await erc20Contract.balanceOf(addr1.address))
				.to.equal(ethers.utils.parseEther('10000000000'));
		});
	...
	describe('Influencers', function () {
		...
		beforeEach(async function() {
			...
			await expect(erc20Contract.transfer(influencerContract.address, ethers.utils.parseEther('50000000000')))
				.to.emit(influencerContract, 'TokenReceive')
				.withArgs(ethers.utils.parseEther('50000000000'));
		});
		...
		it ('Reverts execution of withdraw function when paused', async function() {
			await expect(influencerContract.pause())
				.to.emit(influencerContract, 'Paused')
				.withArgs(owner.address);
			await expect(influencerContract.withdraw())
				.to.be.revertedWith('Pausable: paused');
		})
		it ('Reverts execution of availableAmount function when paused', async function() {
			await expect(influencerContract.pause())
				.to.emit(influencerContract, 'Paused')
				.withArgs(owner.address);
			await expect(influencerContract.availableAmount())
				.to.be.revertedWith('Pausable: paused');
		})
		it('owner is able to change a releaseTime', async function() {
			await expect(influencerContract.setReleaseTime(new Date(2021, 1, 18).getTime() / 1000))
		...
		it('owner successfully claim tokens', async function() {
			await expect(influencerContract.setReleaseTime(new Date(2021, 1, 18).getTime() / 1000))
				...
				.withArgs(new Date(2021, 1, 18).getTime() / 1000);
			await expect(influencerContract.withdraw())
				.to.emit(influencerContract, 'TokenWithdraw')
				.withArgs(ethers.utils.parseEther('50000000000'));
			expect(await erc20Contract.balanceOf(beneficiary.address))
