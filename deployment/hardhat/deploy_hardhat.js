async function main() {
    ...
    const teamVestingContract = await TeamVestingContract.deploy(misBlockBase.address);
    await teamVestingContract.deployed();
    console.log("Team Vesting address:", teamVestingContract.address);

    const StakingContract = await ethers.getContractFactory('StakingContract');
    const stakingContract = await StakingContract.deploy(misBlockBase.address, beneficiary.address, new Date(2022, 2, 15).getTime() / 1000);
    ...
    const marketingContract = await MarketingContract.deploy(misBlockBase.address);
    ...
    const influencerContract = await InfluencerContract.deploy(misBlockBase.address, beneficiary.address, new Date(2021, 9, 17).getTime() / 1000);
    ...
    const presaleContract = await PresaleContract.deploy(misBlockBase.address);
    await influencerContract.deployed();
    ...
    const farmingRewardContract = await FarmingRewardContract.deploy(misBlockBase.address, beneficiary.address);
    await influencerContract.deployed();
    ...
    const developmentFundContract = await DevelopmentFundContract.deploy(misBlockBase.address);
    await influencerContract.deployed();
